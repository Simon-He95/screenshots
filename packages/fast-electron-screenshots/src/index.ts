...
export default class Screenshots extends Events {
  ...
  private nodeScreenshotsPromise?: Promise<typeof import('node-screenshots') | undefined>;

  private nodeScreenshotsUnavailable = false;
  ...
  constructor(opts?: ScreenshotsOpts) {
    super();
    this.logger = opts?.logger || debug('fast-electron-screenshots');
    this.singleWindow = opts?.singleWindow || false;
    this.listenIpc();
    this.$view.webContents.loadURL(
      `file://${require.resolve('fast-react-screenshots/electron/electron.html')}`,
    );
    this.preloadNodeScreenshots();
    if (opts?.lang) {
  ...
  public async startCapture(): Promise<void> {
    ...
    // Kick off image capture ASAP and wait for the renderer while it runs
    const capturePromise = this.capture(display);
    // Initialize the window in parallel with the capture process
    const createWindowPromise = this.createWindow(display);
    await this.isReady;
    const imageBuffer = await capturePromise;
    await createWindowPromise;

    const imageData = Screenshots.bufferToTransferable(imageBuffer);
  ...
  private async resetIfNeeded(force = false) {
    if (!force && !this.shouldReset) {
      return;
    }
  ...
  private preloadNodeScreenshots(): void {
    this.loadNodeScreenshots();
  }

  private loadNodeScreenshots(): Promise<typeof import('node-screenshots') | undefined> {
    if (this.nodeScreenshotsUnavailable) {
      return Promise.resolve(undefined);
    }

    if (!this.nodeScreenshotsPromise) {
      this.nodeScreenshotsPromise = import('node-screenshots')
        .catch((error) => {
          this.logger('SCREENSHOTS:load node-screenshots failed %o', error);
          this.nodeScreenshotsUnavailable = true;
          this.nodeScreenshotsPromise = Promise.resolve(undefined);
          return undefined;
        });
    }

    return this.nodeScreenshotsPromise;
  }

  private static bufferToTransferable(buffer: Buffer): {
  ...
  private async capture(display: Display): Promise<Buffer> {
    ...
    const nodeScreenshots = await this.loadNodeScreenshots();

    if (!this.nodeScreenshotsUnavailable && nodeScreenshots?.Monitor) {
      try {
