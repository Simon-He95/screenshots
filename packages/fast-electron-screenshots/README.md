...
{
  externals: {
    'fast-electron-screenshots': 'require("fast-electron-screenshots")'
  }
}
```
...
module.exports = {
  ...
  pluginOptions: {
    electronBuilder: {
      // 不打包，使用 require 加载
      externals: ["fast-electron-screenshots"],
    },
  },
};
```
...
export default defineConfig({
  plugins: [
    viteExternals({
      "fast-electron-screenshots": 'require("fast-electron-screenshots")', // 模块名称: 全局变量
    }),
  ],
});
```
...
export interface ScreenshotsOpts {
  lang?: Lang;
  // 调用日志，默认值为 debug('fast-electron-screenshots')
