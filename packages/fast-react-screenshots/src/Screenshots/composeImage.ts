...
export default function composeImage ({ image, imageWidth, imageHeight, width, height, history, bounds }: ComposeImageOpts): Promise<Blob> {
  return new Promise<Blob>((resolve, reject) => {
    ...
    const rx = (imageWidth || width) / width
    const ry = (imageHeight || height) / height
