...
import { Bounds, ImageDataPayload } from '../Screenshots/types'
...
export default function App (): JSX.Element {
  ...
  const [imageData, setImageData] = useState<ImageDataPayload | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    const onCapture = (display: Display, payload: ImageDataPayload) => {
      setDisplay(display)
      setImageData({
    ...
    const notifyReset = () => {
      if (typeof queueMicrotask === 'function') {
        queueMicrotask(() => window.screenshots.reset())
      } else {
        Promise.resolve().then(() => window.screenshots.reset())
      }
    }
